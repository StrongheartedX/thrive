...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const choreStackResponse = await apiClient.chores.choreStackLoad({
  ...
  if (timePlanReason === "for-time-plan") {
    ...
    if (!query.timePlanActivityRefId) {
      throw new Response("Missing Time Plan Activity Ref Id", { status: 500 });
    }
  }

  return json({
    date: query.date,
    choreStack: choreStackResponse.chore_stack,
  });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (timePlanReason === "for-time-plan") {
      ...
      if (!query.timePlanActivityRefId) {
        throw new Response("Missing Time Plan Activity Ref Id", {
          status: 500,
        });
      }
    }
    ...
    await apiClient.timeEvents.timeEventInDayBlockCreateForChoreStack({
...
export default function TimeEventInDayBlockCreateForChoreStack() {
  ...
  const inputsEnabled =
    navigation.state === "idle" && !loaderData.choreStack.archived;
      ...
      >
        ...
        <FormControl fullWidth>
          ...
          <OutlinedInput
            label="name"
            name="name"
            defaultValue={loaderData.choreStack.name}
            readOnly={true}
          />
        </FormControl>
