...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  try {
    const result = await apiClient.habits.habitStackLoad({
      ref_id: id,
      allow_archived: true,
    });
    ...
    return json({
      ...
      habits: result.habits as Array<Habit>,
      tags: result.tags,
      note: result.note,
  ...
  try {
    switch (form.intent) {
      case "update": {
        ...
        return redirect(`/app/workspace/apps/habits/stacks`);
      }

      case "create-note": {
        await apiClient.notes.noteCreate({
          owner: noteStdOwner(NamedEntityTag.HABIT_STACK, id),
          content: [],
        });

        return redirect(`/app/workspace/apps/habits/stacks/${id}`);
      }

      case "archive": {
        await apiClient.habits.habitStackArchive({
          ref_id: id,
        });
      ...
      case "remove": {
        await apiClient.habits.habitStackRemove({
          ref_id: id,
        });
...
export default function HabitStackView() {
  ...
  const inputsEnabled =
    ...
    !loaderData.habitStack.archived &&
  ...
  return (
    <LeafPanel
      key={`habit-stack-${loaderData.habitStack.ref_id}`}
      entityType={NamedEntityTag.HABIT_STACK}
      entityRefId={loaderData.habitStack.ref_id}
      fakeKey={`habit-stack-${loaderData.habitStack.ref_id}`}
      showArchiveAndRemoveButton
      inputsEnabled={inputsEnabled}
      entityArchived={loaderData.habitStack.archived}
    ...
    >
      ...
      {isWorkspaceFeatureAvailable(
        ...
        <TimeEventInDayBlockStack
          ...
          inputsEnabled={inputsEnabled}
          title="Time Events"
          createLocation={`/app/workspace/calendar/time-event/in-day-block/new-for-habit-stack?habitStackRefId=${loaderData.habitStack.ref_id}`}
          entries={[]}
        />
      )}
      ...
      {isWorkspaceFeatureAvailable(
        ...
        loaderData.timePlanActivities && (
          <SectionCard
            id="habit-stack-time-plans"
            title="Time Plans"
            actions={
              <SectionActions
                id="habit-stack-time-plans-actions"
                topLevelInfo={topLevelInfo}
                inputsEnabled={inputsEnabled}
                actions={[
                  NavSingle({
                    text: "Add",
                    highlight: false,
                    link: `/app/workspace/apps/time-plans/add-habit-stack-to-plans?habitStackRefId=${loaderData.habitStack.ref_id}`,
                  }),
                ]}
              />
            }
          >
            <TimePlanActivityList
              ...
              habitStacksByRefId={
                new Map([[loaderData.habitStack.ref_id, loaderData.habitStack]])
              }
...
export const ErrorBoundary = makeLeafErrorBoundary(
  ...
  ParamsSchema,
  {
    notFound: (params) => `Could not find habit stack with ID ${params.id}!`,
    error: (params) =>
      `There was an error loading habit stack with ID ${params.id}! Please try again!`,
  },
);
