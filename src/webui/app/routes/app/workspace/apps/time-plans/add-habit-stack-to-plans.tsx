...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [habitStackResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      apiClient.habits.habitStackLoad({
        ref_id: query.habitStackRefId,
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `HabitStack:std:${query.habitStackRefId}`,
      }),
    ]);

  return json({
    habitStack: habitStackResult.habit_stack,
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateHabitStackWithPlan({
          habit_stack_ref_id: query.habitStackRefId,
        ...
        return redirect(
          `/app/workspace/apps/habits/stacks/${query.habitStackRefId}`,
        );
      }
...
export default function AddHabitStackToPlans() {
  ...
  return (
    <LeafPanel
      key="add-habit-stack-to-plans"
      fakeKey={`add-habit-stack-to-plans/${searchParams.get("habitStackRefId")}`}
      returnLocation={`/app/workspace/apps/habits/stacks/${searchParams.get("habitStackRefId")}`}
      returnLocationDiscriminator="add-habit-stack-to-plans"
...
export const ErrorBoundary = makeLeafErrorBoundary(
  (params, searchParams) =>
    `/app/workspace/apps/habits/stacks/${searchParams.get("habitStackRefId")}`,
  ParamsSchema,
  {
    error: () =>
      `There was an error adding the habit stack to time plans! Please try again!`,
  },
);
