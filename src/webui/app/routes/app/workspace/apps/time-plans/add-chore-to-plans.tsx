...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [choreResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      apiClient.chores.choreLoad({
        ref_id: query.choreRefId,
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `Chore:std:${query.choreRefId}`,
      }),
    ]);

  return json({
    chore: choreResult.chore,
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateChoreWithPlan({
          chore_ref_id: query.choreRefId,
...
export default function AddChoreToPlans() {
...
export const ErrorBoundary = makeLeafErrorBoundary(
  ...
  {
    error: () =>
      `There was an error adding the chore to time plans! Please try again!`,
  },
);
