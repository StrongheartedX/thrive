...
import { CREATE_CHORE } from "@jupiter/core/apps/time_plans/store/mutations/create-entities";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const [timePlanResult, summaries, stacks] = await Promise.all([
...
// A new chore made from the time plan view, as a leaf of the plan rather than
// a trip to its app: making it doesn't reload the plan.
export default function TimePlanNewChore() {
  ...
  return (
    <LeafPanel
      key={`time-plan-${id}/new-chore`}
      fakeKey={`time-plan-${id}/new-chore`}
