...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [habitResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `Habit:std:${query.habitRefId}`,
      }),
    ]);
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateHabitWithPlan({
...
export default function AddHabitToPlans() {
...
export const ErrorBoundary = makeLeafErrorBoundary(
  ...
  {
    error: () =>
      `There was an error adding the habit to time plans! Please try again!`,
  },
);
