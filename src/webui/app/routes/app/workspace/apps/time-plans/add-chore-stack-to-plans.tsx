...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [choreStackResult, timePlansResult, timePlanActivitiesResult] =
    await Promise.all([
      apiClient.chores.choreStackLoad({
        ref_id: query.choreStackRefId,
      ...
      apiClient.timePlans.timePlanActivityFindForTarget({
        allow_archived: false,
        target: `ChoreStack:std:${query.choreStackRefId}`,
      }),
    ]);

  return json({
    choreStack: choreStackResult.chore_stack,
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    switch (form.intent) {
      case "add": {
        await apiClient.timePlans.timePlanAssociateChoreStackWithPlan({
          chore_stack_ref_id: query.choreStackRefId,
        ...
        return redirect(
          `/app/workspace/apps/chores/stacks/${query.choreStackRefId}`,
        );
      }
...
export default function AddChoreStackToPlans() {
  ...
  return (
    <LeafPanel
      key="add-chore-stack-to-plans"
      fakeKey={`add-chore-stack-to-plans/${searchParams.get("choreStackRefId")}`}
      returnLocation={`/app/workspace/apps/chores/stacks/${searchParams.get("choreStackRefId")}`}
      returnLocationDiscriminator="add-chore-stack-to-plans"
...
export const ErrorBoundary = makeLeafErrorBoundary(
  (params, searchParams) =>
    `/app/workspace/apps/chores/stacks/${searchParams.get("choreStackRefId")}`,
  ParamsSchema,
  {
    error: () =>
      `There was an error adding the chore stack to time plans! Please try again!`,
  },
);
