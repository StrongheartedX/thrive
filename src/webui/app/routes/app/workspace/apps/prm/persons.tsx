...
import { UpcomingCatchUpsWidget } from "@jupiter/core/apps/prm/sub/person/component/upcoming-catch-ups-widget";
...
export default function Persons() {
  ...
  return (
    <TrunkPanel
      key={"persons"}
      createLocation="/app/workspace/apps/prm/persons/new"
          ...
          actions={[
            NavMultipleSpread({
              navs: [
                NavSingle({
                  text: "Circles",
                  link: `/app/workspace/apps/prm/circles`,
                  icon: <GroupWorkIcon />,
                }),
              ],
            }),
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              {filteredEntries.length === 0 && (
                <EntityNoNothingCard
                  ...
                  newEntityLocations="/app/workspace/apps/prm/persons/new"
...
function PersonCard({ entry, circlesByRefId, topLevelInfo }: PersonCardProps) {
  return (
    <EntityCard entityId={`person-${entry.person.ref_id}`}>
      ...
      />
      <EntityLink to={`/app/workspace/apps/prm/persons/${entry.person.ref_id}`}>
