import type { EntityId, ChoreStack } from "@jupiter/webapi-client";
...
import { sortChoreStacksNaturally } from "#/core/apps/chores/root";

interface ChoreStackSelectSingleProps {
  name: string;
  label: string;
  allowNone?: boolean;
...
export function ChoreStackSelectSingle(props: ChoreStackSelectSingleProps) {
  const allStacksByRefId = useMemo(
    () => new Map(props.allStacks.map((stack) => [stack.ref_id, stack])),
    [props.allStacks],
  );

  const sortedStacks = sortChoreStacksNaturally(props.allStacks);
...
function selectedStackToOption(
  ...
  allStacksByRefId: Map<EntityId, ChoreStack>,
) {
...
function stackToOption(stack: ChoreStack) {
  return {
