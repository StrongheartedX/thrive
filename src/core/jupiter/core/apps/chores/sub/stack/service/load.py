"""Shared service for loading a chore stack and its dependent entities."""
...
@use_case_result
class ChoreStackLoadResult(UseCaseResultBase):
...
class ChoreStackLoadService:
    ...
    async def do_it(
        ...
        *,
        ...
        """Load a chore stack together with the entities that hang off it."""
        chore_stack = await uow.get_for(ChoreStack).load_by_id(
            chore_stack.ref_id, allow_archived=allow_archived
        )
        owner_link = EntityLink.std(
            NamedEntityTag.CHORE_STACK.value, chore_stack.ref_id
        )

        aspect = await uow.get_for(Aspect).load_by_id(chore_stack.aspect_ref_id)
        chapter = (
            await uow.get_for(Chapter).load_by_id(chore_stack.chapter_ref_id)
            if chore_stack.chapter_ref_id
            else None
        )
        goal = (
            await uow.get_for(Goal).load_by_id(chore_stack.goal_ref_id)
            if chore_stack.goal_ref_id
            else None
        )
