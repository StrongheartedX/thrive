"""A chore stack — a group of chores done together."""
...
class ChoreStack(LeafEntity):
    ...
    tag_link = OwnsAtMostOne(
        TagLink, owner=IsEntityLinkStd(NamedEntityTag.CHORE_STACK.value)
    )
    contact_link = OwnsAtMostOne(
        ContactLink, owner=IsEntityLinkStd(NamedEntityTag.CHORE_STACK.value)
    )
    location_link = OwnsAtMostOne(
        LocationLink, owner=IsEntityLinkStd(NamedEntityTag.CHORE_STACK.value)
    )
    note = OwnsAtMostOne(Note, owner=IsEntityLinkStd(NamedEntityTag.CHORE_STACK.value))
    publish_entity = OwnsAtMostOne(
        PublishEntity, owner=IsEntityLinkStd(NamedEntityTag.CHORE_STACK.value)
    )
    ...
    def new_chore_stack(
        ...
        """Create a chore stack."""
        return ChoreStack._create(
            ctx,
    ...
    def update(
        ...
        """Update a chore stack's properties. Period cannot change after creation."""
