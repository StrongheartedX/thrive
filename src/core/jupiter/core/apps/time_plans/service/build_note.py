"""Build an unsaved time plan note from standard questions, aspects, and goals."""
...
def _aspect_heading(aspect: Aspect, aspects_by_ref_id: dict[EntityId, Aspect]) -> str:
    """Heading for an aspect in a time plan note."""
...
def _goal_heading(
    ...
    """Heading for a goal in a time plan note."""
...
def _build_note_content(
    ...
    """Build time plan note content from selected questions and life plan sections."""
...
class BuildNoteService:
    """Build a time plan note from standard questions, aspects, and goals."""
    ...
    async def do_it(
        ...
        *,
        ...
        final_include_aspects = period_allows_aspects_in_note(time_plan.period) and (
            include_aspects
            if include_aspects is not None
            else time_plan_domain.include_aspects_in_note
        )
        final_include_goals = period_allows_goals_in_note(time_plan.period) and (
            include_goals
            if include_goals is not None
            else time_plan_domain.include_goals_in_note
        )
