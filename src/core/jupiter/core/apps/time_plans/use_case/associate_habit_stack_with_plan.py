"""Use case for creating time plan activities for a habit stack."""
...
@use_case_args
class TimePlanAssociateHabitStackWithPlanArgs(JupiterCreateCrownEntityArgs):
    """Args."""
...
@use_case_result
class TimePlanAssociateHabitStackWithPlanResult(UseCaseResultBase):
    """Result."""
...
class TimePlanAssociateHabitStackWithPlanUseCase(
    JupiterCreateCrownEntityUseCase[
    ...
    async def _perform_transactional_mutation(
        ...
        args: TimePlanAssociateHabitStackWithPlanArgs,
    ) -> TimePlanAssociateHabitStackWithPlanResult:
        """Execute the command's actions."""
        if len(args.time_plan_ref_ids) == 0:
            raise InputValidationError("You must specify some time plans")

        habit_stack = await LoadForAclService().do_it(
            uow,
            HabitStack,
            args.habit_stack_ref_id,
            context.user.ref_id,
            AccessLevel.READER,
        )
        ...
        for time_plan in time_plans:
            if not time_plan.allows_inbox_tasks:
                raise InputValidationError(
                    f"Time plan {time_plan.name} does not allow habit stack activities"
                )
        ...
        return TimePlanAssociateHabitStackWithPlanResult(
    ...
    async def _perform_post_transactional_mutation_work(
        ...
        context: JupiterLoggedInMutationContext,
        args: TimePlanAssociateHabitStackWithPlanArgs,
        result: TimePlanAssociateHabitStackWithPlanResult,
    ) -> None:
        ...
        async with self._ports.domain_storage_engine.get_unit_of_work() as uow:
            ...
            member_habits = await uow.get_for(Habit).find_all_generic(
                parent_ref_id=None,
                allow_archived=False,
                stack_ref_id=args.habit_stack_ref_id,
            )
