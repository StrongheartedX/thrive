...
@use_case_args
class HabitStackFindArgs(JupiterFindCrownEntityArgs):
...
@use_case_result_part
class HabitStackFindResultEntry(UseCaseResultBase):
...
@use_case_result
class HabitStackFindResult(UseCaseResultBase):
    """The result."""

    entries: list[HabitStackFindResultEntry]


@readonly_use_case(WorkspaceFeature.HABITS)
class HabitStackFindUseCase(
    JupiterFindCrownEntityUseCase[HabitStackFindArgs, HabitStackFindResult]
):
    """The command for finding habit stacks."""
    ...
    async def _perform_transactional_read(
        ...
        include_habits = args.include_habits or False
        ...
        if not habit_stacks:
            return HabitStackFindResult(entries=[])

        stack_owner_links = [
            EntityLink.std(NamedEntityTag.HABIT_STACK.value, stack.ref_id)
            for stack in habit_stacks
        ]
        ...
        return HabitStackFindResult(
            entries=[
                HabitStackFindResultEntry(
                    habit_stack=stack,
                    habits=(
                        habits_by_stack_ref_id.get(stack.ref_id, [])
                        if include_habits
                        else None
                    ),
                    aspect=(
